...
/*   Updated: 2019/01/27 13:16:08 by sjimenez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				handle_args(t_ssl *h, int ac, char **av)
{
	...
	h->file_path = (char**)ft_memalloc(sizeof(char*) * (h->file_ct + 1));
	while (++i < h->file_ct)
		h->file_path[i] = ft_strdup(av[i + j + 1]);
	h->file_txt = (char**)ft_memalloc(sizeof(char*) * (h->file_ct + 1));
	if (h->options & OPT_S)
	{
		if (!(h->options & OPT_P))
		{
			h->file_txt[0] = ft_strdup(h->file_path[0]);
			h->file_path[0] = ft_strjoin_free("\"", h->file_path[0], 0);
			h->file_path[0] = ft_strjoin_free(h->file_path[0], "\"", 0);
		}
		else
		{
			h->file_txt[1] = ft_strdup(h->file_path[1]);
			h->file_path[1] = ft_strjoin_free("\"", h->file_path[1], 2);
			h->file_path[1] = ft_strjoin_free(h->file_path[1], "\"", 1);
		}
	}
